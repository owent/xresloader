...
| UeCsv-UProperty  | UnrealEngine配置支持的字段属性                                    | 字段分组            | 蓝图权限     | 编辑权限    | *可选*,默认值: XResConfig\|BlueprintReadOnly\|EditAnywhere                                                     |
| UeCsv-CaseConvert| 是否开启驼峰命名转换（默认开启）                                  | true/false          |              |             | *可选*,开启后将使用首字母大写的驼峰命名法生成字段名和类名                                                          |
