...
var initDataSource = function () {
    G.currentID = mapSource2ID[dataSrcImplInstance.getCurrentFileName() + "|" + dataSrcImplInstance.getCurrentTableName()];
}
    ...
    for(var key in curMsg) {
        ...
        if (fd.isMapField()) {
            ...
            if (mapValueDesc.getType() == PBType.MESSAGE) {
                if (fieldMessageType.getName() == "combine_id") {
                    ...
                    for(var subKey in value) {
                        newKV[actualKey][subKey] = toCombineID(value[subKey].prefix, value[subKey].id);
                    }
                } else {
            ...
            if (fieldMessageType.getName() == "combine_id") {
                ...
                for(var subKey in value) {
                    newKV[actualKey].add(subKey, toCombineID(value[subKey].prefix, value[subKey].id));
                }
            } else {
                for(var subKey in value) {
                    generateCombineID(curMsg[key], fieldMessageType);
...
var currentMessageCallback = function (curMsg, msgDesc) {
    generateCombineID(curMsg, msgDesc);
